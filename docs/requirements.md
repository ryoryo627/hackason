...
**UC-2: 新規患者登録** — Admin UI 3ステップフォーム。登録ボタン1つでFirestore書込からSlackチャンネル作成・メンバー招待まで全自動
...
**UC-6: Admin UIでの確認・エクスポート** — 検索・フィルタ → 患者詳細 → 3形式エクスポート

**UC-7: ナレッジベース管理** — ドキュメント追加 → チャンク確認 → 検索テスト → エージェント別カテゴリバインド
...
- ✅ M-17: RAGナレッジベース — CRUD + Embedding検索
- ✅ M-18: 定時タスク — エンドポイント + cron認証
- ✅ M-19: デプロイ + デモデータ整備
- ✅ マスタ管理・組織設定画面
...
| 画面 | 説明 | 詳細参照 |
|------|------|---------|
| ログイン | Firebase Auth | ui-ux-design.md |
| セットアップウィザード | Slack連携初期設定 | ui-ux-design.md |
| ダッシュボード | 統計・要対応アラート・接続状態・最近の報告 | ui-ux-design.md |
| 患者一覧 | 検索・フィルタ・ソート・グループ表示 | ui-ux-design.md |
| 患者登録 | 3ステップフォーム → 自動Slack連携 | ui-ux-design.md |
| 患者詳細 | BPSサマリー・タイムライン・推奨事項・エクスポート | ui-ux-design.md |
| 患者情報編集 | 基本情報・タグ編集 | ui-ux-design.md |
| アラート一覧 | 緊急度・確認状態フィルタ | ui-ux-design.md |
| アラート詳細 | 根拠報告・推奨アクション・確認操作 | ui-ux-design.md |
| ナレッジベース | ドキュメント管理・チャンク確認・検索テスト | ui-ux-design.md |
| ナレッジ追加 | PDF/MD/URL追加・チャンク設定 | ui-ux-design.md |
