...
| 15 | ユーザー管理 | `/settings/users` Sidebar + Main | システム設定 | ✅ |
...
**リスクバッジ:** AI自動判定のリスクレベルには「AI」インジケータを表示（`risk_level_source = "auto"` の場合）。
...
**ヘッダー:** 患者名・年齢・性別 / リスクバッジ（AI自動判定時は「AI」表示+判定理由） / 基礎疾患チップ / Slackチャンネルリンク / 編集ボタン
...
**リスク変更履歴カード:** アラートカードの下に配置。リスクレベル変更履歴（自動/手動バッジ、変更理由、トリガー、アラートスナップショット）をタイムライン形式で表示。

**患者編集画面の警告:** リスクレベルがAI自動管理中（`risk_level_source = "auto"`）の場合、リスクレベル選択欄に「現在AIが自動判定中です。手動で変更すると自動ディエスカレーションが停止します。」と警告を表示。
...
Notion/Linear風のモノクロベース + ネイビーアクセントのライトテーマ。

**アクセント（Navy #2B4C7E — 11段階グラデーション）:**

| トークン | Hex | 用途 |
|---------|------|------|
| accent-50 | #F0F4FA | 極薄背景 |
| accent-100 | #DCE6F5 | テキスト選択 |
| accent-200 | #B8CDE9 | — |
| accent-500 | #4373BD | リンク、アイコン |
| accent-600 | #2B4C7E | プライマリボタン、タブインジケータ |
| accent-900 | #122038 | 選択テキスト色 |

**背景:**

| トークン | Hex | 用途 |
|---------|------|------|
| bg-primary | #FFFFFF | カード・メインエリア |
| bg-secondary | #FAFAFA | ページ背景・サイドバー |
| bg-tertiary | #F5F5F5 | アイコン背景・入力無効状態 |
| bg-hover | #F0F0F0 | ホバー状態 |
| bg-active | #EBEBEB | アクティブ/押下状態 |
...
| text-secondary | #6B6B6B | 補助テキスト・アイコン |
| text-tertiary | #9B9B9B | ラベル・キャプション |
| text-placeholder | #B4B4B4 | プレースホルダー |

**ボーダー:**

| トークン | Hex | 用途 |
|---------|------|------|
| border-light | #F0F0F0 | 区切り線 |
| border | #E8E8E8 | カードボーダー |
| border-strong | #D4D4D4 | ホバー時ボーダー |

**ステータス:**

| 用途 | カラー | Hex |
|------|--------|-----|
| 成功 / LOW・接続OK | グリーン | #10B981 |
| 警告 / MEDIUM | イエロー | #F59E0B |
| 危険 / HIGH | レッド | #EF4444 |
...
| トークン | Hex | 用途 |
|---------|------|------|
| icon-default | #6B6B6B | 標準アイコン色（= text-secondary） |
| icon-muted | #9B9B9B | 控えめアイコン色（= text-tertiary） |
| icon-subtle-bg | #F5F5F5 | アイコン背景色（= bg-tertiary） |

### 6.2 アイコンデザイン原則 — Quiet Icons

アイコンはコンテンツの視認性を優先し、モノクロニュートラルスタイルで統一する。

**原則:**
- アイコン背景: `bg-bg-tertiary`（#F5F5F5）に統一。カテゴリ別の色分け（accent/warning/success等）は使用しない
- アイコン色: `text-text-secondary`（#6B6B6B）に統一。情報の主役はテキストとデータであり、アイコンは控えめに機能する
- アイコンサイズ: 統計カード `w-5 h-5`、セクションヘッダー `w-4 h-4`、インライン `w-3.5 h-3.5`
- **例外**: ステータスアイコン（接続OK/NG）のみ `text-success` / `text-danger` を使用

**適用箇所:** ダッシュボード統計カード、BPSセクション、エージェント設定、ナレッジベース統計、患者詳細の各種カード

### 6.3 UIコンポーネント（`components/ui/` — 14個）

| コンポーネント | 用途 |
|--------------|------|
| Alert | 通知バナー（成功・警告・エラー・情報） |
| Badge | リスクレベル・ステータス表示（HIGH/MEDIUM/LOWカラーコード、AI自動判定時は「AI」インジケータ付き） |
| Button | アクションボタン（variant: primary/secondary/danger/ghost）。ghostはホバー時 `bg-bg-hover` |
| Card | コンテンツコンテナ。interactive時はホバーで `border-strong` + `shadow-sm`。CardHeaderは `leading-tight` |
...
| Tabs | タブ切替（アクティブ時 `accent-600` ボーダー下線） |
| TagInput | 複数選択タグ入力（FilterPillパターン） |
| Textarea | テキストエリア |
| Toast | トースト通知 |

### 6.4 レイアウトコンポーネント（`components/layout/` — 6個）
...
| useDebounce | `hooks/useDebounce.ts` | 汎用デバウンスフック（検索入力等） |
