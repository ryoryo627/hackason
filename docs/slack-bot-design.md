# Slack Bot仕様書

## 1. チャンネル構成

| チャンネル | 命名規則 | 用途 | 作成タイミング |
|-----------|----------|------|---------------|
| `#oncall-night` | 固定名 | 夜間オンコール。朝8時定時レポート | 初期セットアップ時 |
| `#pt-{患者名}` | `pt-` + 患者名の空白除去 | 患者ごとの情報蓄積・AI対話・アラート | Admin UI患者登録時 |

## 2. Bot権限（OAuth Scopes — 15個）

```
channels:manage      # チャンネル作成・管理
channels:read        # チャンネル情報取得
channels:join        # Botのチャンネル参加
channels:history     # パブリックチャンネルのメッセージ履歴取得
chat:write           # メッセージ投稿
groups:write         # プライベートチャンネル操作
groups:read          # プライベートチャンネル情報取得
groups:history       # プライベートチャンネルのメッセージ履歴取得
im:history           # ダイレクトメッセージ履歴取得
mpim:history         # グループDM履歴取得
users:read           # メンバー情報取得
users:read.email     # メンバーメールアドレス取得
files:read           # 添付ファイル読み取り
reactions:write      # リアクション追加（報告確認用）
app_mentions:read    # @bot メンション検知
```

## 3. Events API設定

### 3.1 Subscribe Events

| Event | 用途 |
|-------|------|
| `message.channels` | パブリックチャンネルのメッセージ受信 |
| `message.groups` | プライベートチャンネルのメッセージ受信 |
| `app_mention` | @bot メンション受信 |

### 3.2 Request URL

`https://homecare-bot-xxx.run.app/slack/events`

### 3.3 署名検証

すべてのリクエストで `X-Slack-Signature` ヘッダーと `X-Slack-Request-Timestamp` を使用してHMAC-SHA256署名を検証。タイムスタンプの許容差は5分以内。

```python
import hashlib, hmac, time

def verify_slack_signature(signing_secret, timestamp, body, signature):
    if abs(time.time() - int(timestamp)) > 300:
        return False
    sig_basestring = f"v0:{timestamp}:{body}"
    my_signature = "v0=" + hmac.new(
        signing_secret.encode(), sig_basestring.encode(), hashlib.sha256
    ).hexdigest()
    return hmac.compare_digest(my_signature, signature)
```

## 4. インタラクションルール

### 4.1 3モードインタラクション

| ユーザーアクション | Botの挙動 | データ保存 |
|-------------------|----------|-----------|
| チャンネルに直接投稿 | **無反応**（監視しない） | 保存しない |
| Botアンカーメッセージにリプライ | Intake Agent起動 → BPS構造化 → 確認応答 | **保存する** |
| `@bot` で質問/コマンド | Context/Summary Agent → 回答 | 保存しない |

### 4.2 イベント判定フロー

```python
def handle_event(event):
    # 1. Bot自身の投稿は無視
    if event.get("bot_id"):
        return

    channel_id = event["channel"]
    patient = find_patient_by_channel(channel_id)
    if not patient:
        return  # 患者チャンネルでない場合は無視

    # 2. @bot メンション
    if event["type"] == "app_mention":
        text = event["text"]
        if "サマリー" in text or "経過" in text or "引き継ぎ" in text:
            return summary_agent.run(patient, text)
        elif "保存" in text:
            return intake_agent.run_save_mode(patient, channel_id)
        else:
            return context_agent.run(patient, text)

    # 3. アンカーメッセージへのリプライ判定
    if event.get("thread_ts") == patient.slack_anchor_message_ts:
        return intake_agent.run(patient, event)

    # 4. それ以外（直接投稿）→ 無視
    return
```

### 4.3 添付ファイル処理

```python
def process_attachments(event, patient):
    files = event.get("files", [])
    for file in files:
        if file["mimetype"] == "application/pdf":
            # PDFダウンロード → GCS保存 → テキスト抽出
            content = download_and_extract_pdf(file)
        elif file["mimetype"].startswith("image/"):
            # 画像ダウンロード → GCS保存 → Gemini Vision
            content = download_and_extract_image(file)
        else:
            continue

        # BPS構造化
        return intake_agent.run_with_content(patient, content, file)
```

## 5. アンカーメッセージ仕様

患者チャンネル作成時にBotが自動投稿する初期メッセージ。

```
📋 {患者名}さん（{年齢}歳・{性別}）のチャンネルです。

📌 使い方:
• このメッセージにリプライ → 情報が保存されます（テキスト/PDF/画像）
• @HomeCare AI + 質問 → AIが患者コンテキストに基づいて回答します
• チャンネルに直接投稿 → 保存されません（チーム内の会話用）

🏷️ {基礎疾患リスト}
🏥 {事業所} | 📍 {地区}
```

チャンネルトピック:
```
{患者名}（{年齢}歳・{性別}）| {基礎疾患} | {事業所} | {地区}
```

## 6. メッセージフォーマット集

### 6.1 報告確認応答

```
✅ 保存しました

Bio: SpO2 94%(↓2pt/1w), 食欲低下
Psycho: 表情暗い、意欲低下の可能性
Social: -

📊 確信度: 0.92 | 報告者: 看護師A | ソース: テキスト
```

### 6.2 アラート投稿

```
⚠️ 複合的悪化徴候を検知しました

🫀 Bio: SpO2低下トレンド(96%→94%/1w) + 食欲低下
🧠 Psycho: 意欲低下の可能性
💊 服薬: 降圧薬アドヒアランス低下(週2回忘れ)

📋 推奨アクション:
• 呼吸器感染症・心不全増悪の除外検査を検討
• 主治医への早期相談を推奨

📎 根拠: 看護師A(2/3 14:30), 薬剤師B(2/4 10:15)
```

### 6.3 BPSサマリー

```
📋 田中太郎さん BPSサマリー（2026-02-05時点）

🫀 Biological
• SpO2: 94%(↓2pt/1w) — 要経過観察
• 食欲: 低下（2/3〜）
• 服薬: 降圧薬アドヒアランス低下（週2回忘れ）
• BP 148/92 安定、HbA1c 7.2%(↑0.3/3m)

🧠 Psychological
• 表情暗い、意欲低下傾向
• 認知変化の可能性（家族報告あり）

👥 Social
• 妻(82歳)と二人暮らし。介護負担増加傾向
• 訪看週2回、デイサービス週1回

⚠️ 注意点
• SpO2低下+食欲低下 → 呼吸器感染症/心不全増悪の除外を推奨
• 認知変化+服薬低下 → 認知機能評価・服薬管理方法見直しを検討
```

### 6.4 朝8時定時レポート（#oncall-night）

```
📊 朝レポート | 2026-02-05（水）

状態変化: 5名 / 全24名

🔴 HIGH (2名)
• 田中太郎(85歳) - SpO2低下+食欲低下+服薬低下+認知変化 → #pt-田中太郎
• 村上トメ(96歳) - 褥瘡悪化+心不全増悪徴候 → #pt-村上トメ

🟡 MED (3名)
• 佐藤美智子(78歳) - 血糖コントロール悪化 → #pt-佐藤美智子
• 後藤実(73歳) - 腹水増加傾向 → #pt-後藤実
• 林誠(92歳) - 心不全症状微増 → #pt-林誠

🟢 変化なし: 19名
```

### 6.5 Context Agent回答例

```
💬 田中太郎さんについてお答えします。

直近1週間のBPSデータに基づくと:

🫀 SpO2は2/3の訪問時に94%で、1週間前の96%から低下傾向にあります。
食欲低下も2/3から報告されており、COPD急性増悪の初期段階の可能性を
考慮する必要があります。

💊 薬剤師Bさんの報告(2/4)では降圧薬のアドヒアランス低下が指摘されて
おり、認知変化との関連も検討すべきです。

📋 推奨:
• 呼吸器感染症・心不全増悪の除外（胸部X線、BNP等）
• 認知機能スクリーニング（MMSE or HDS-R）
• 服薬カレンダーの導入検討

📎 参照: 看護師A(2/3), 薬剤師B(2/4), ご家族(2/1)
```

## 7. Slack APIエンドポイント使用一覧

| 用途 | API メソッド | 呼び出し元 |
|------|-------------|-----------|
| チャンネル作成 | `conversations.create` | 患者登録API |
| Botチャンネル参加 | `conversations.join` | 患者登録API |
| メッセージ投稿 | `chat.postMessage` | 全エージェント |
| スレッドリプライ | `chat.postMessage` (thread_ts付き) | Intake/Context/Summary |
| メンバー招待 | `conversations.invite` | 患者登録API |
| トピック設定 | `conversations.setTopic` | 患者登録/編集API |
| チャンネルアーカイブ | `conversations.archive` | 患者アーカイブAPI |
| ファイルダウンロード | `files.info` + URL取得 | Intake Agent |
| ユーザー情報取得 | `users.info` | reporter_name解決 |
| 接続テスト | `auth.test` | セットアップウィザード |

## 8. Slack Appマニフェスト

Admin UIのセットアップウィザードでコピーして使用するマニフェストJSON:

```json
{
  "display_information": {
    "name": "HomeCare AI",
    "description": "在宅医療支援AIエージェント",
    "background_color": "#4ecdc4"
  },
  "features": {
    "bot_user": {
      "display_name": "HomeCare AI",
      "always_online": true
    }
  },
  "oauth_config": {
    "scopes": {
      "bot": [
        "channels:manage",
        "channels:read",
        "channels:join",
        "channels:history",
        "chat:write",
        "groups:write",
        "groups:read",
        "groups:history",
        "im:history",
        "mpim:history",
        "users:read",
        "users:read.email",
        "files:read",
        "reactions:write",
        "app_mentions:read"
      ]
    }
  },
  "settings": {
    "event_subscriptions": {
      "request_url": "{CLOUD_RUN_URL}/slack/events",
      "bot_events": [
        "message.channels",
        "message.groups",
        "app_mention"
      ]
    },
    "org_deploy_enabled": false,
    "socket_mode_enabled": false
  }
}
```
